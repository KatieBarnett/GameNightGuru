...
If you haven't yet, download and install Android Studio. It is best to have the latest stable version from the [Android Studio download page](https://developer.android.com/studio)
...
> {.special} **Note:** If you decide to use a physical device and connect it to your computer with a cable you can skip this step!

It can take a while for the emulator image to download, so while you are waiting, create a new emulator. 

> {.special} **Note:** For this codelab, you will need to use a System Image of **36.0** (codename Baklava) and above. 
>  
...
You can also clone it from [github](https://github.com/android/appfunctions):
- Clone the official App Functions repository using your terminal:
  ```bash
...
> {.special} **Note:** You can also add them to the version catalog `libs.versions.toml` like the other dependencies in this project if you want.
...
- the maximum time the game should take (`maxDurationMinutes`)
...
> {.special} **Note:** There are also imports at the top of the file, make sure to uncomment those as well, I have not included them above to make it easier to read 

> What is `@RequiresApi(Build.VERSION_CODES.BAKLAVA)`? This is indicating that this code will only work with API 36 and above (codenamed Baklava). It will be ignored on devices with lower APIs.
...
> {.special} **Note:** If you have a lot of processing to do, it can be a good idea to switch to suitable coroutine dispatcher so the UI thread is not blocked. We don't need to worry about it here as `getAllGames`in `GameRepository` already use the `IO` dispatcher as part of the database integration. 
...
> {.special} **Note:** After installation, you may need to restart your terminal or Android Studio for the `android` command to be recognised.
...
This downloads the metadata and instructions that Gemini needs to understand how to write and optimise App Functions for you and will add it to the directory for all AI agents that you have installed.
...
I have deliberately made this prompt very specific so that the output is what we want, but feel free to play around with it and see what results you get!

![app_functions_with_skill](assets/app_functions_with_skill.gif)
...
2.  Click the **Keep** button to keep the code in `GameAppFunctions.kt`. If you don't like it, you can click **Revert** and it will be removed and you can try prompting again.

![keep_changes](assets/keep_changes.png)

3.  If any imports are missing (appearing in red), click on them and press `Alt+Enter` (Windows) or `Option+Enter` (Mac) to add them.
   
![add_import](assets/add_import.png)

> {.special} **Important:** Ensure Gemini also generates a `PlayRecord` data class at the bottom of the file. It must be annotated with `@AppFunctionSerializable(isDescribedByKDoc = true)` for the AI agent to understand the output (just like we had with `GameSummary`!

```kotlin
/**  
 * A summary of a logged play. 
 */
@AppFunctionSerializable(isDescribedByKDoc = true)  
data class PlayRecord(  
    /** The name of the game played */  
    val gameName: String,  
    /** The date the game was played, as a timestamp in milliseconds */  
    val date: Long,  
    /** The number of players */  
    val numPlayers: Int,  
    /** The play time in minutes */  
    val playTimeMinutes: Int,  
    /** The rating given to the play */  
    val rating: Float  
)
```

Your `addPlay` function should now be ready to use, of course, because this is Generative AI, it may look a bit different to the sample code, but should hopefully function the same. 
...
2. Scroll down and tap **About phone**. 
   _(Note: On Samsung devices, you will then need to tap **Software information**)._
...
> {.warning} **Warning:** If you have two devices listed, then you will run into some issues later where you have to specify which device to run the commands on. I recommend disconnecting one so you only have one device listed - it is much easier!
...
> {.special} **Note:** Make sure to replace the package name if you have updated it to your own name
>
> You can also modify the number after `-A` in the `grep` command (the_ `--after-context` flag) to see more or less of the function definition.
...
*Be aware  -  if you are writing your own App Functions, anyone else with a device connected to `adb` or an app that acts as an agent can see all your function definitions/schemas, so don’t include language or proprietary information that you don’t want others to see!*
...
Because this is a `.sh` shell script, it will not run in standard Command Prompt or PowerShell. You must run this command using **Git Bash** (which is installed automatically if you have Git for Windows) or WSL (Windows Subsystem for Linux). If you used the Android Studio Terminal to install the Android CLI, you can switch the terminal dropdown in Android Studio from "PowerShell" to "Git Bash" right in the IDE.

![run_privileged](assets/run_privileged.gif)
...
> {.warning} **Warning:** Never commit your API key to public source control. Store it locally in your `local.properties` file or in a secure location like a Github secret variable. 
...
Now ask Gemini to add a play of one of the games then go and check to see if it has worked in the app.
